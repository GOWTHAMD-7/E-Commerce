...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] flex flex-col w-full">
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#E5E5E7]/80 w-full px-6 py-4 md:px-12 md:py-4 flex flex-col gap-4 shadow-sm transition-all duration-300">
        <div className="flex items-center justify-between gap-4 w-full">
          {/* Left Section: Brand Logo, Greeting & Address */}
          <div className="flex items-center gap-4 shrink-0">
            <Link to="/" style={{ textDecoration: 'none' }}>
              <h1 className="text-2xl md:text-4xl font-extrabold tracking-[-0.03em] text-[#111113] m-0 leading-none select-none" style={{ fontFamily: '"Playfair Display", "Cormorant Garamond", serif' }}>
                Sellora
              </h1>
            </Link>

            {auth.user && (
              <span className="hidden sm:inline-block text-sm font-semibold text-[#111113]/85 select-none bg-slate-100/60 px-2.5 py-1 rounded-full border border-slate-200/30">
                Hi, {auth.user.email.split('@')[0].charAt(0).toUpperCase() + auth.user.email.split('@')[0].slice(1)}
              </span>
            )}
            ...
            {auth.user && (auth.user.role === 'CUSTOMER' || auth.user.role === 'SELLER') && (
              <button 
                ...
                className="flex items-center gap-2.5 px-3.5 py-1.5 bg-[#F4F4F5] border border-slate-200/30 hover:border-slate-300 hover:bg-white rounded-full transition-all duration-200 group text-left shadow-sm focus:outline-none"
              >
                <span className="flex items-center justify-center w-5.5 h-5.5 rounded-full bg-white text-[#2563EB] shadow-xs shrink-0 group-hover:bg-[#F4F4F5]/50 transition-colors">
                ...
                <div className="flex flex-col text-[10px] leading-tight pr-1 select-none shrink-0">
                  <span className="text-[8px] text-[#6E6E73] font-extrabold uppercase tracking-wider">Deliver to</span>
                  <span className="text-[11px] font-bold text-[#111113]">
          ...
          {/* Center Section: Global Search Bar (Desktop/Tablet) */}
          ...
          <div className="flex items-center gap-4 md:gap-6 shrink-0">
            ...
            <button
              ...
              className="md:hidden p-2 text-slate-500 hover:text-[#111113] hover:bg-slate-50 rounded-full transition-colors focus:outline-none"
              title="Toggle Search"
            >
            ...
            {/* Cart, Wishlist Buttons: only for CUSTOMER or SELLER */}
            {auth.user && (auth.user.role === 'CUSTOMER' || auth.user.role === 'SELLER') && (
              <>
                {/* 2. Wishlist (Outlined Button) */}
                <button 
                  onClick={() => navigate('/favorites')} 
                  className="flex items-center gap-2 px-4 py-2 border border-[#E5E5E7] hover:border-[#111113] hover:bg-slate-50/50 rounded-full text-xs font-semibold text-[#111113] bg-white transition-all duration-200 group focus:outline-none"
                >
                  <svg className="w-4 h-4 text-slate-500 group-hover:text-rose-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                  </svg>
                  <span className="hidden lg:inline">Wishlist</span>
                  {favorites.length > 0 && (
                    <span className="flex h-4 min-w-4 px-1 items-center justify-center text-[9px] font-bold text-white bg-[#111113] rounded-full shrink-0">
                      {favorites.length}
                    </span>
                  )}
                </button>

                {/* 3. Cart (Dark Pill Button) */}
                <button 
                  onClick={() => navigate('/cart')} 
                  className="relative flex items-center gap-2 px-5 py-2.5 text-xs font-semibold text-white bg-[#111113] hover:bg-[#222225] hover:shadow-md active:scale-95 rounded-full transition-all duration-200 group focus:outline-none"
                >
                  <svg className="w-4.5 h-4.5 text-indigo-100 group-hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                  </svg>
                  <span className="hidden sm:inline">Cart</span>
                  {totalCartDistinctItems > 0 && (
                    <span className="flex h-5 min-w-5 px-1.5 items-center justify-center text-[10px] font-extrabold text-[#111113] bg-white rounded-full transition-all duration-200 shrink-0">
            ...
            {/* Seller Hub & Admin Hub (Desktop buttons, mobile dropdown links) */}
            {auth.user && auth.user.role === 'SELLER' && (
              <button 
                onClick={() => navigate('/seller/dashboard')} 
                className="hidden sm:flex items-center gap-2 px-4.5 py-2 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-250 rounded-full transition-all duration-200"
              >
                Seller Hub
              </button>
            )}
            ...
            {auth.user && auth.user.role === 'ADMIN' && (
              <button 
                ...
                className="hidden sm:flex items-center gap-2 px-4.5 py-2 text-xs font-semibold text-[#111113] bg-white border border-[#E5E5E7] hover:border-[#111113] rounded-full transition-all duration-200"
              >
                Admin Hub
              </button>
            )}

            {/* 4. User Profile Dropdown or Login/Register */}
            {auth.user ? (
              <div className="relative">
                <button
                  onClick={() => setShowProfileDropdown(prev => !prev)}
                  className="flex items-center justify-center w-9 h-9 rounded-full bg-[#111113] hover:bg-[#222225] text-white font-extrabold text-xs border border-[#111113]/10 transition-all focus:outline-none shadow-sm"
                  title="User Menu"
                >
                ...
                {showProfileDropdown && (
                  <div className="absolute right-0 mt-3 w-60 bg-white border border-[#E5E5E7] rounded-2xl shadow-xl py-3 z-50 text-left">
                    <div className="px-4 pb-2.5 border-b border-slate-100 flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-[#FAFAFA] text-[#111113] border border-[#6E6E73]/10 flex items-center justify-center font-bold text-xs shrink-0">
                        {auth.user.email.substring(0, 2).toUpperCase()}
                      </div>
                    ...
                    <div className="pt-2 px-1.5 flex flex-col gap-0.5">
                      <button
                        onClick={() => {
                      ...
                      {auth.user.role === 'SELLER' && (
                        <button
                          ...
                          className="sm:hidden flex items-center gap-2.5 w-full px-3 py-2 text-xs font-semibold text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all text-left focus:outline-none"
                        >
                      ...
                      {auth.user.role === 'ADMIN' && (
                        <button
                          ...
                          className="sm:hidden flex items-center gap-2.5 w-full px-3 py-2 text-xs font-semibold text-slate-655 hover:bg-slate-50 rounded-xl transition-all text-left focus:outline-none"
                        >
                      ...
                      <button
                        ...
                        className="flex items-center gap-2.5 w-full px-3 py-2 text-xs font-semibold text-rose-650 hover:bg-rose-50 hover:text-rose-700 rounded-xl transition-all text-left border-t border-slate-100 mt-1.5 pt-2 focus:outline-none"
                      >
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className="px-4 py-2 text-xs font-semibold text-white bg-[#111113] hover:bg-[#222225] hover:shadow-sm active:scale-95 rounded-full transition-all focus:outline-none"
                >
                  Login
                </button>
                <button 
                  onClick={() => navigate('/register')} 
                  className="hidden sm:inline-block px-4 py-2 text-xs font-semibold text-[#111113] bg-white hover:bg-slate-50 border border-[#E5E5E7] rounded-full transition-all focus:outline-none"
                >
                  Register
                </button>
              </div>
        ...
        {isMobileSearchOpen && (
          <div className="md:hidden w-full pb-1 transition-all duration-300 animate-in fade-in slide-in-from-top-2">
            <SearchBar searchQuery={searchQuery} setSearchQuery={setSearchQuery} />
          </div>
        )}
      ...
      <main className="w-full max-w-[1300px] mx-auto px-6 py-8 flex-grow">
        <Routes>
        {/* Public Routes */}
        <Route 
          path="/" 
